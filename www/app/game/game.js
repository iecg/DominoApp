...
.controller('GameCtrl', ['$scope', '$ionicModal', '$ionicPopup', '$state', '$window', '$ionicHistory', '$timeout', function($scope, $ionicModal, $ionicPopup, $state, $window, $ionicHistory, $timeout){
  ...
  $scope.checkWinner = function(){
    if($scope.totalScore($scope.game.teams[$scope.teamId].scores) >= $scope.game.maxscore){
      ...
      var myPopup = $ionicPopup.show({
        title: $scope.game.teams[$scope.teamId].members[0].full_name + " and " + $scope.game.teams[$scope.teamId].members[1].full_name + ' won',
  ...
  $scope.updateMatchHistory = function(){
    if($scope.game.teams[0].members[0].full_name === "Player 1"){ return; }
    var matches = {"teams":[{"members":[],"score":""},{"members":[],"score":""}],"date":null};
    matches.date = new Date();
    if($scope.teamId === 0){
      matches.teams[0].members.push($scope.game.teams[0].members[0].full_name);
      matches.teams[0].members.push($scope.game.teams[0].members[1].full_name);
      matches.teams[0].score = $scope.totalScore($scope.game.teams[0].scores);
      matches.teams[1].members.push($scope.game.teams[1].members[0].full_name);
      matches.teams[1].members.push($scope.game.teams[1].members[1].full_name);
      ...
      matches.teams[0].members.push($scope.game.teams[1].members[0].full_name);
      matches.teams[0].members.push($scope.game.teams[1].members[1].full_name);
      matches.teams[0].score = $scope.totalScore($scope.game.teams[1].scores);
      matches.teams[1].members.push($scope.game.teams[0].members[0].full_name);
      matches.teams[1].members.push($scope.game.teams[0].members[1].full_name);
  ...
  $scope.updatePlayer = function(player){
    var index = $scope.players.findIndex(function(player){
      return player.full_name === this.full_name;
    }, player);
